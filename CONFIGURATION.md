...
**⚠️ ВАЖНО:** В конфигурационных файлах нет реальных паролей. Все пароли настраиваются через переменные хоста в Foreman.

### Настройка паролей в Foreman

1. **Пароль root**: Устанавливается автоматически при создании хоста через Foreman
2. **Пароль LUKS (`crypto_passphrase`)**: Устанавливается через переменную хоста

**Пример настройки переменной хоста для LUKS:**

```
crypto_passphrase = TestPassword  # ⚠️ ЗАМЕНИТЕ на реальный надежный пароль!
```

**⚠️ ОБЯЗАТЕЛЬНО:**
- Замените `TestPassword` на реальный надежный пароль для LUKS
- Используйте длинные случайные пароли (минимум 16 символов)
- Не используйте одинаковые пароли для разных хостов
- Храните пароли в безопасном месте (менеджер паролей)

### Где используются пароли

- **Root пароль**: Настраивается в шаблоне через `<%= root_password %>` (Foreman автоматически генерирует)
- **LUKS passphrase**: Передается через переменную хоста `crypto_passphrase` и используется в:
  - `d-i partman-crypto/passphrase password`
  - `luks_tpm_enroll.sh` скрипт для регистрации в TPM2
